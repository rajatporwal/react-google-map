...
export default function WithGoogleApi(props) {
  ...
  const apiKey = "AIzaSyCi2lKlHegqpC-MaYP_ZPgb_uIF1ylgVcc";
  return (
...
export function withGmapsContext(BaseComp) {
  const comp = React.forwardRef((props, ref) => (
    <GmapsContext.Consumer>
      {(apiKey) => <BaseComp {...props} apiKey={apiKey} ref={ref} />}
    </GmapsContext.Consumer>
  ));
...
export const getGmapsMarkerInstance = (markerComponentRef) =>
  markerComponentRef.current.state[SECRET_MARKER_KEY];
